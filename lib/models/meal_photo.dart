...
class MealPhoto {
  ...
  /// 解析の確からしさ 'high' / 'low'。低いときは利用者に選んでもらう
  final String? aiConfidence;

  /// 答えが割れたときの候補(JSON配列の文字列)。先頭が採用中のもの
  final String? aiCandidates;
  ...
  /// 解析が割れていて、利用者に選んでもらいたい状態か。
  /// 手で直したものは確定済みとして扱う(選び直しを促さない)
  bool get needsNameReview =>
      aiConfidence == 'low' && userCorrectedName == null;

  /// 候補の一覧。保存されていなければ空
  List<String> get candidateNames {
    final raw = aiCandidates;
  ...
  factory MealPhoto.fromMap(Map<String, dynamic> map) {
    return MealPhoto(
      ...
      aiCandidates: map['ai_candidates'] as String?,
  ...
  MealPhoto copyWith({
    ...
    return MealPhoto(
      ...
      aiCandidates:
          clearCandidates ? null : (aiCandidates ?? this.aiCandidates),
      userCorrectedName:
